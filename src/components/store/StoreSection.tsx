                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pe-2 custom-scrollbar">
                  ...
                  {categories?.map(cat => (
                    <label key={cat} className="flex items-center gap-3 cursor-pointer group" onClick={() => handleFilterChange('category', cat)}>
                      ...
                      <span className="text-sm text-[var(--text-muted)] group-hover:text-[var(--text)] transition-colors">{t(`cat_${cat.toLowerCase().replace(/ /g, '_')}`, cat)}</span>
                    </label>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pe-2 custom-scrollbar">
                  ...
                  {cities?.map(city => (
                    <label key={city} className="flex items-center gap-3 cursor-pointer group" onClick={() => handleFilterChange('city', city)}>
                      ...
                      <span className="text-sm text-[var(--text-muted)] group-hover:text-[var(--text)] transition-colors">{t(`city_${city.toLowerCase()}`, city)}</span>
                    </label>
                  ))}
                </div>
              </div>

...
export default function AICreativeStudio({ onAction }: AICreativeStudioProps) {
  ...
  return (
    <div className="p-6 md:p-12 max-w-6xl mx-auto">
      <div className="mb-12">
        <p className="micro-label mb-4">{t('ai_studio_title', 'Creative Studio')}</p>
        <h2 className="text-4xl md:text-6xl font-bold tracking-tighter mb-4 text-[var(--text)]">AI <span className="gold-text italic-serif">{t('ai_studio_creative', 'Creative')}</span> {t('ai_studio_studio', 'Studio')}</h2>
        <p className="text-[var(--text-muted)] text-base md:text-xl">{t('ai_studio_desc', 'Visualize your home improvement projects with state-of-the-art AI.')}</p>
      </div>
          ...
          <div className="card-luxury p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={() => setActiveTool('edit')} className={`p-3 rounded-xl text-sm font-medium transition-all border ${activeTool === 'edit' ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--border)] text-[var(--text-muted)] hover:bg-[var(--text)]/5'}`}>
                {t('ai_tool_edit_image', 'Edit Image')}
              </button>
              ...
              <button onClick={() => setActiveTool('animate')} className={`p-3 rounded-xl text-sm font-medium transition-all border ${activeTool === 'animate' ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--border)] text-[var(--text-muted)] hover:bg-[var(--text)]/5'}`}>
                {t('ai_tool_animate', 'Animate')}
              </button>
            </div>
          </div>

          <div className="card-luxury p-6 space-y-4">
            <h3 className="font-bold text-lg mb-4">{t('ai_config', 'Configuration')}</h3>
            
            {(activeTool === 'edit' || activeTool === 'animate') && (
              <div className="space-y-2">
                <label className="micro-label">{t('ai_upload_ref', 'Upload Reference Image')}</label>
                  ...
                  <div className="border-2 border-dashed border-[var(--border)] rounded-2xl p-8 flex flex-col items-center justify-center gap-3 group-hover:border-[var(--accent)]/50 transition-all">
                    {image ? (
                      ...
                      <>
                        ...
                        <span className="text-xs text-[var(--text-muted)]">{t('ai_upload_click', 'Click or drag to upload')}</span>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="space-y-2">
              <label className="micro-label">{t('ai_prompt', 'Prompt')}</label>
              <textarea 
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder={activeTool === 'image' ? t('ai_prompt_image_placeholder', 'A modern Moroccan living room with zellige tiles...') : t('ai_prompt_edit_placeholder', 'Add a traditional lantern to the ceiling...')}
            ...
            {activeTool === 'image' && (
              <div className="space-y-2">
                <label className="micro-label">{t('ai_image_size', 'Image Size')}</label>
            ...
            {(activeTool === 'video' || activeTool === 'animate') && (
              <div className="space-y-2">
                <label className="micro-label">{t('ai_aspect_ratio', 'Aspect Ratio')}</label>
                <div className="flex gap-2">
                  {(['16:9', '9:16'] as const).map(r => (
                    <button key={r} onClick={() => setAspectRatio(r)} className={`flex-1 py-2 rounded-lg text-xs font-bold border transition-all ${aspectRatio === r ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--border)] text-[var(--text-muted)]'}`}>
                      {r === '16:9' ? t('ai_landscape', 'Landscape') : t('ai_portrait', 'Portrait')}
                    </button>
                  ))}
                </div>
              </div>
            ...
            >
              ...
              {loading ? t('ai_processing', 'Processing...') : t('ai_gen_magic', 'Generate Magic')}
            </button>
          </div>
        </div>
          ...
          <div className="card-luxury p-8 min-h-[500px] flex flex-col items-center justify-center relative overflow-hidden">
            {loading ? (
              <div className="text-center space-y-4">
                ...
                <p className="text-[var(--text-muted)] animate-pulse">{t('ai_crafting', 'Our AI is crafting your masterpiece...')}</p>
              </div>
            ) : result ? (
              <div className="w-full h-full flex flex-col gap-4">
                ...
                >
                  <Download size={18} /> {t('ai_download', 'Download')}
                </button>
              </div>
            ) : videoOp ? (
              <div className="w-full h-full flex flex-col items-center justify-center gap-6">
                {!videoOp.done ? (
                  <div className="text-center space-y-4">
                    ...
                    <p className="font-bold text-xl">{t('ai_generating_video', 'Generating Video...')}</p>
                    <p className="text-[var(--text-muted)] max-w-xs mx-auto">{t('ai_veo_processing', 'Veo is processing your request. This usually takes 1-2 minutes.')}</p>
                  </div>
                ) : (
                  <div className="w-full space-y-4">
                    ...
                    />
                    <p className="text-sm text-[var(--text-muted)] text-center italic">{t('ai_video_success', 'Video generated successfully with Veo 3.1')}</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="text-center space-y-6 max-w-md">
                ...
                <h4 className="text-2xl font-bold">{t('ai_creation_appear', 'Your creation will appear here')}</h4>
                <p className="text-[var(--text-muted)]">{t('ai_creation_desc', 'Use the tools on the left to start visualizing your dream home projects.')}</p>
              </div>
            )}
          </div>
        </div>

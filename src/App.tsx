...
const MaterialsMarketplacePage = lazy(() => import('./pages/MaterialsMarketplacePage'));
const LandingPage = lazy(() => import('./pages/LandingPage'));

const Home = lazy(() => import('./pages/Home'));
const FindArtisan = lazy(() => import('./pages/FindArtisan'));
...
export default function App() {
  ...
  const [customerTab, setCustomerTab] = useState<'home' | 'find' | 'store' | 'bookings' | 'account' | 'creative'>(() => (sessionStorage.getItem('m3allem_customerTab') as any) || 'home');
  const [artisanTab, setArtisanTab] = useState<'dashboard' | 'requests' | 'services' | 'wallet' | 'profile'>(() => (sessionStorage.getItem('m3allem_artisanTab') as any) || 'dashboard');
  const [activeTab, setActiveTab] = useState(() => sessionStorage.getItem('m3allem_activeTab') || 'dashboard');

  useEffect(() => { sessionStorage.setItem('m3allem_customerTab', customerTab); }, [customerTab]);
  useEffect(() => { sessionStorage.setItem('m3allem_artisanTab', artisanTab); }, [artisanTab]);
  useEffect(() => { sessionStorage.setItem('m3allem_activeTab', activeTab); }, [activeTab]);

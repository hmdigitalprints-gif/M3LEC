...
export default function FindM3allem() {
  ...
  }, [selectedCategory, selectedCity, searchQuery, minRating, minPrice, maxPrice, isOnline]);
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-8">
          ...
          <div className="relative group">
            ...
            >
              {categories?.map(cat => <option key={cat} value={cat} className="bg-[var(--card-bg)]">{cat === 'All' ? t('find_ar_all', 'All') : t(`cat_${cat.toLowerCase().replace(/ /g, '_')}`, cat)}</option>)}
            </select>
          ...
          <div className="relative group">
            ...
            <select 
              ...
              className="w-full appearance-none bg-[var(--card-bg)]/50 border border-[var(--border)] rounded-[32px] py-6 ps-16 pe-8 text-lg text-[var(--text)] focus:outline-none focus:border-[var(--accent)]/50 transition-all cursor-pointer"
            >
              {cities?.map(city => <option key={city} value={city} className="bg-[var(--card-bg)]">{city === 'All' ? t('find_ar_all', 'All') : t(`city_${city.toLowerCase()}`, city)}</option>)}
            </select>
            <ChevronDown className="absolute end-6 top-1/2 -translate-y-1/2 text-[var(--text-muted)]/50 pointer-events-none" size={20} />
          </div>
        </div>

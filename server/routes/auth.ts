...
router.post("/register/artisan", async (req, res) => {
  try {
    ...
    if (DISABLE_OTP) {
      ...
      setTokenCookie(res, token);
      return res.status(201).json({ message: "Registration successful", userId: result.id, user: { id: result.id, name, role: 'artisan', verified: true } });
    }
...
router.post("/register/seller", async (req, res) => {
  try {
    ...
    if (DISABLE_OTP) {
      ...
      setTokenCookie(res, token);
      return res.status(201).json({ message: "Registration successful", userId: result.id, user: { id: result.id, name, role: 'seller', verified: true } });
    }
...
router.post("/register/company", async (req, res) => {
  try {
    ...
    if (DISABLE_OTP) {
      ...
      setTokenCookie(res, token);
      return res.status(201).json({ message: "Registration successful", userId: result.id, user: { id: result.id, name, role: 'company', verified: true } });
    }

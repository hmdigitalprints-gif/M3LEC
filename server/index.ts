...
async function startServer() {
  ...
  const io = new Server(httpServer, {
    cors: {
      origin: process.env.CORS_ORIGIN || "https://m3allem.ma",
    },
  });
  ...
  app.use("/api", (req, res, next) => {
    if (!dbReady) {
      if (req.method === 'GET' && req.path === '/marketplace/categories') {
        return res.json([
          { id: '1', name_en: 'Plumbing', name_fr: 'Plomberie', name_ar: 'سباكة', icon: 'wrench', image_url: 'https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&q=80', description: 'Plumbing services', popular: true, groupId: null },
          { id: '2', name_en: 'Electrical', name_fr: 'Électricité', name_ar: 'كهرباء', icon: 'zap', image_url: 'https://images.unsplash.com/photo-1621905252507-b35492cc74b4?auto=format&fit=crop&q=80', description: 'Electrical services', popular: true, groupId: null }
        ]);
      }
      ...
      if (req.method === 'POST' && req.path === '/auth/login') {
         return res.json({
           token: 'dummy-token',
           user: { id: 'dummy-id', name: 'Demo User', role: 'customer', phone: '+1234567890', verified: true }
         });
      }
      ...
      return res.status(503).json({ 
        ...
        message: "The application is unable to connect to the database. Please check your PostgreSQL configuration in the Settings menu." 
      });
    }
    next();
  });
